...
$(function() {
    ...
    var sections = {
        ...
        zenarmor: {
            ...
            description: 'Documented Zenarmor API, reporting export and log metadata for application, TLS, device, policy and security context.'
        },
        netflow: {
            icon: 'fa-random',
    ...
    var de = {
        ...
        form: {
            ...
            'JSON lines': 'JSON-Zeilen',
            'Key-value syslog': 'Key-Value-Syslog',
            ...
            'Zenarmor API timeout seconds': 'Zenarmor-API-Timeout in Sekunden',
            'Verify Zenarmor API TLS': 'Zenarmor-API-TLS pruefen',
            'Import Zenarmor applications': 'Zenarmor-Anwendungen importieren',
            'Import Zenarmor categories': 'Zenarmor-Kategorien importieren',
            'Import Zenarmor TLS metadata': 'Zenarmor-TLS-Metadaten importieren',
            'Import Zenarmor session context': 'Zenarmor-Session-Kontext importieren',
            'Import Zenarmor policy actions': 'Zenarmor-Policy-Aktionen importieren',
            'Import Zenarmor device context': 'Zenarmor-Device-Kontext importieren',
            'Import Zenarmor security events': 'Zenarmor-Security-Events importieren',
            ...
            'Web, application and security categories for hunting, correlation and explainability.': 'Web-, Anwendungs- und Sicherheitskategorien fuer Hunting, Korrelation und Erklaerbarkeit.',
            'SNI, TLS version and JA3/JA4 values when present in exported reporting data.': 'SNI, TLS-Version und JA3/JA4-Werte, wenn sie in exportierten Reporting-Daten vorhanden sind.',
            'Session IDs, byte counters, ASN and country values when available.': 'Session-IDs, Byte-Zaehler, ASN und Laenderwerte, wenn vorhanden.',
            'Block, allow, policy and rule names from documented exports.': 'Block-, Allow-, Policy- und Regelnamen aus dokumentierten Exporten.',
            'Device ID, device name and user labels when present.': 'Device-ID, Geraetename und Benutzerlabels, wenn vorhanden.',
            'Threat names, security categories and SASE/secure-access event labels when present.': 'Threat-Namen, Sicherheitskategorien und SASE-/Secure-Access-Eventlabels, wenn vorhanden.',
